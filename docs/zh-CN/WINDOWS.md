...
如果你明确要在 Windows 本机运行样本，可以使用专家 ETW 动态观测模式：
...
这个模式不是沙箱。它会在本机真实执行目标文件，并只保存受限大小的 JSON 摘要，不保存原始 `.etl` 日志。详见 [Windows ETW 专家动态观测](WINDOWS_ETW_OBSERVATION.md)。

如果目标程序需要管理员权限，报告会记录 `failure_reason=requires_elevation`。这时可以用管理员 PowerShell 运行命令，或使用 GUI 的提权提示。建议 GUI 本身尽量保持非管理员运行：Windows 会阻止普通资源管理器向管理员窗口拖拽文件。

高风险强壳样本仍建议使用专用虚拟机、专业沙箱，或导入 Sysmon/Windows Event Log 证据。`observe linux-docker` 只用于 Linux ELF 样本，不运行 Windows PE 文件。
