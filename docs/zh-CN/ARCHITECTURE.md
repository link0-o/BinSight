...
扫描器负责控制证据提取。LLM 不决定是否解析二进制或运行可选增强工具；它作为第二评估者接收结构化扫描结果和检索到的知识上下文。报告会分开保留本地评估、AI 评估和最终融合评估。
...
- 内置字符串分类器属于 **Temporary / Prototype / Educational Implementation** triage helper；它会保守降噪，不能当作行为证明。
...
- `StringScanner`：分类可疑字符串，并把配置字符串与风险证据分开。
- `RiskRuleEngine`：使用 `yaml-cpp` 匹配本地 YAML 规则、证据组合、风险类型、置信度和证据强度。
