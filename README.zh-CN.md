...
BinSight 是一个用于扫描可执行文件并生成风险分析报告的 C++20 CLI。它采用“确定性扫描管线 + 本地规则 + 本地 RAG + 可选 LLM 评估”的结构：程序负责固定、可审计的取证流程，报告会分开保留本地评估、AI 评估和最终融合结论。
...
- 默认安全的静态模式，以及需要显式确认风险的 Linux Docker 和 Windows ETW 专家动态观测模式。
- 支持三种分析模式：
  - `none`：离线规则分析，不调用模型。
...
Windows ETW 专家动态观测会在本机真实执行目标文件。它不是沙箱，不能阻止恶意行为。第一版只写出受限大小的 JSON 摘要，不保存原始 `.etl` 日志，因此不会像 VM 或完整 Procmon 日志那样膨胀到几十 GB。详见 [Windows ETW 专家动态观测](docs/zh-CN/WINDOWS_ETW_OBSERVATION.md)。
...
启用在线 AI Provider 且使用默认 `--report-lang both` 时，BinSight 会分别为中文 Markdown 和英文 Markdown 请求对应语言的 AI 评估文本。这样可能产生不止一次模型请求，但可以保证两份人工报告各自语言一致。
Windows 版会给中文 Markdown 报告写入 UTF-8 BOM，方便 Windows 记事本和其他编辑器自动识别编码，减少 ANSI/GBK 误判导致的乱码。Linux/macOS 版保持无 BOM UTF-8。
...
模型下拉框支持手动输入，因此厂商新增模型但 preset 还没更新时，可以直接填真实 model id。AI 配置页还提供轻量“测试模型联通”功能，只发送一条很短的联通测试提示，不会上传二进制报告。慢模型可能需要几十秒，默认 AI 超时为 90 秒，可以在 GUI、配置向导或命令行 `--llm-timeout` 中修改。
